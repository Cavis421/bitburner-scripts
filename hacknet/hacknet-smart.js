...
export async function main(ns) {
    ...
    if (mode !== "hack") {
        ns.tprint("?? Only 'hack' mode is implemented for now. Using hacking-focused aug bundle.");
    }

    ns.tprint(`?? hacknet-smart started (mode=${mode}, spend=${(spendFraction*100).toFixed(0)}% money, pause at ${(stopFraction*100).toFixed(0)}% of aug bundle).`);
    ...
    while (true) {
        ...
        if (bundleCost > 0) {
            ...
            ns.print("?? No suitable aug bundle found (maybe no factions / rep). Proceeding with normal Hacknet upgrades.");
        }
