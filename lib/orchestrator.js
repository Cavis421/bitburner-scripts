...
export function getScriptCost(ns, script, host = "home", threads = 1) {
  if (!ns.fileExists(script, host)) return Infinity;
  return ns.getScriptRam(script, host) * Math.max(1, Math.floor(threads));
}
...
export function isScriptRunning(ns, script, host = "home") {
  return ns.ps(host).some(p => p.filename === script);
}
...
export function getScriptProcesses(ns, script, host = "home") {
  return ns.ps(host).filter(p => p.filename === script);
}
...
export function killScripts(ns, host, scriptsSet) {
  ...
  const set = scriptsSet instanceof Set ? scriptsSet : new Set(scriptsSet || []);
...
export function tryStart(ns, script, {
  ...
  if (!ns.fileExists(script, host)) return { ok: false, reason: "missing-file" };

  const t = Math.max(1, Math.floor(threads));
  const cost = getScriptCost(ns, script, host, t);
  const spendable = getSpendableRam(ns, host, reserveRam);
  ...
  let pid = 0;
  try {
    if (host === "home") pid = ns.run(script, t, ...(args || []));
    else pid = ns.exec(script, host, t, ...(args || []));
  } catch (_e) {
    pid = 0;
  }
...
export function ensureDaemon(ns, script, {
  ...
  if (!script) return { action: "skipped", ok: false, reason: "no-script" };
  if (!ns.fileExists(script, host)) return { action: "skipped", ok: false, reason: "missing-file" };

  const running = ns.ps(host).filter(p => p.filename === script);

  if (running.length > 0) {
    if (!requireArgsMatch) return { action: "noop", ok: true };
    const want = (args || []).map(String);
    const match = running.some(p => arraysEqual(p.args.map(String), want));
  ...
  const res = tryStart(ns, script, { host, threads, args, reserveRam });
